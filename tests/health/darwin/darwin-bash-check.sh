...
test_script_exists() {
    local test_name="macOS health check script exists"
...
test_script_executable() {
    local test_name="macOS health check script is executable"
...
test_script_execution() {
    local test_name="macOS health check script executes without errors"
    local exit_code=0
...
test_script_output() {
    local test_name="macOS health check script returns 'pong'"
    local actual_output
...
test_no_stderr() {
    local test_name="macOS health check script produces no stderr output"
    local stderr_output
...
test_os_detection() {
    local test_name="macOS health check script validates OS correctly"
    local exit_code=0
    ...
    if [[ "$(uname -s)" == "Darwin" ]]; then
        ...
        if [[ ${exit_code} -ne 0 ]]; then
            write_test_result "${test_name}" "FAIL" "Script failed on macOS system"
            return 1
        fi
        
        write_test_result "${test_name}" "PASS"
    else
        write_test_result "${test_name}" "SKIP" "Not running on macOS"
    fi
...
run_all_tests() {
    printf "\n+++++++++++++++++++++++++++++++++++++\n"
    printf "macOS Health Check Test Suite\n"
    printf "+++++++++++++++++++++++++++++++++++++\n\n"
